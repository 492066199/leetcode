package com.sailing.leetcode.solution309;
...
public class Solution {
    ...
    public int maxProfit(int[] prices) {
        ...
        flag = new boolean[prices.length];

        return maxP(prices, 0);
    }

    private int maxP(int[] prices, int start) {
        ...
        //第一次成交
        for(int i = start + 2; i <= prices.length; i++){
            int left_profit = maxPp(prices, start, i - 1);
            int right = maxP(prices, i + 1);
            max = Math.max(right + left_profit, max);
            if(right + left_profit == 12){
...
//                System.out.println(left_profit);
    ...
    private int maxPp(int[] prices, int start, int end) {
